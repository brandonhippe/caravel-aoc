...
sv_blocks=$(shell find systemverilog -iname "*.sv")
.PHONY: convert-sv
convert-sv: $(sv_blocks)
	@echo "Converting SystemVerilog to Verilog:"
	@echo "$(sv_blocks)"
	@if [ ! -d "$(HOME)/sv2v" ]; then \
		echo "Installing sv2v..."; \
		git clone https://github.com/zachjs/sv2v.git $(HOME)/sv2v && cd $(HOME)/sv2v && make; \
	fi

	@for sv_block in $(sv_blocks); do \
		sv2v --write=$(PWD)/verilog/rtl $$sv_block; \
	done